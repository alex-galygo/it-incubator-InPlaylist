//data for player
export const playlists = [
    {
        type: 'Playlist',
        playlist_crc: './player/images/playlist-1-logo.png',
        title: 'Hip-Hop Hits',
        count_songs: 4,
        duration_all: '12m 13s',
        authors: ['Eminem', '50cent', 'Lloyd Banks'],
        tracks: [
            {
                cover_image: './player/images/eminem.png',
                title: 'Rap God',
                author: ' Eminem',
                src: 'https://tonuz.net/uploads/music/2023/10/Eminem_Rap_God.mp3',
                inTrand: true,
            },
            {
                cover_image: './player/images/50-cent.png',
                title: 'In da Club',
                author: '50 cent',
                src: 'https://muzfan.net/uploads/files/2020-07/1594719752_50-cent-in-da-club.mp3',
                inTrand: false,
            },
            {
                cover_image: './player/images/dmx.png',
                title: 'X Gon\' Give It To Ya',
                author: 'DMX',
                src: 'https://cdn.muzonchek.ru/files/music/3026/DMX-feat-The-xx_-_XX-Gon-Give-It-To-Ya.mp3',
                inTrand: false,

            },
            {
                cover_image: './player/images/lyud.png',
                title: 'You Don\'t Know',
                author: 'Eminem feat 50 Cent, Cashis, Lloyd Banks',
                src: 'https://cdn.muzonchek.ru/files/music/67784/Eminem-50-Cent-Cashis-Lloyd-Banks_-_You-Dont-Know.mp3',
                inTrand: false,
            }
        ]
    },
    {
        type: 'Playlist',
        playlist_crc: './player/images/playlist-2-logo.png',
        title: 'Rap Hits 1990s',
        count_songs: 4,
        duration_all: 12.13,
        authors: ['Public Enemy', 'Vanila Ice', 'MC Hammer'],
        tracks: [
            {
                cover_image: './player/images/public-enemy.png',
                title: 'Fight the Power',
                author: 'Public Enemy',
                src: 'https://tonuz.net/uploads/music/2023/10/Eminem_Rap_God.mp3',
                inTrand: true,

            },
            {
                cover_image: './player/images/vanila-ice.png',
                title: 'Ice Ice Baby',
                author: 'Vanila Ice',
                src: 'https://muzfan.net/uploads/files/2020-07/1594719752_50-cent-in-da-club.mp3',
                inTrand: true

            },
            {
                cover_image: './player/images/Hammer.png',
                title: 'You Can’t Touch This',
                author: 'MC Hammer',
                src: 'https://cdn.muzonchek.ru/files/music/3026/DMX-feat-The-xx_-_XX-Gon-Give-It-To-Ya.mp3',
                inTrand: false,

            },
            {
                cover_image: './player/images/brand-nubian.png',
                title: 'Brand Nubian',
                author: 'Brand Nubian',
                src: 'https://cdn.muzonchek.ru/files/music/67784/Eminem-50-Cent-Cashis-Lloyd-Banks_-_You-Dont-Know.mp3',
                inTrand: true,

            },
        ]
    },
];
